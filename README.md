...
**SD Gundam G Generation DS**（任天堂 DS，日版）完整独立汉化构建系统
...
错位；改用半角 `()` 后单行显示更紧凑。同时订正了若干与官方译名不一致或
存在歧义的机体名（如「精神感应高达」系列应为「精神力高达」、
「W高达零式(EW)」与其飞翼形态应予以区分）。

- **译名修改** —— 「精神感应高达」→「精神力高达」、「精神感应高达Mk2」→
  「精神力高达MKⅡ」、「W高达零式(EW)」→「零式飞翼高达EW」、
  「卡碧尼Mk2」→「卡碧尼MKⅡ」等，并同步修正其变体名（普露兹机/普露机）。
- **变体后缀格式统一** —— 将「机体名／变体」「机体名（变体）」统一改为半角
  「机体名(变体)」，覆盖 ∀高达、V2突击破坏、ZZ高达、Z高达、圣盾高达、
  翔翼型强袭、嫣红强袭、MC高达、高达X、卡碧尼、海盗X1、神高达、西斯库德、
  斯摩、赛克艾恩、倒X、迪斯帕达、金牛座、自由高达、灵格斯 等共 30+ 处机体
  条目。
- **新增机体名身份校验门禁** —— 防止合法中文字符串被错误分配给不相邻的
  机体条目（W高达零式换名类问题的回归测试）。
...
```
[build] final ROM sha1 2acc1bc4ca6da5357ee6cb263ea951fe490a1f0e  (MATCHES the shipped translation)
[build] wrote sd-gundam-g-generation-zh.nds  (30,359,400 bytes)
```

追加 `--pad32m 路径` 可同时输出补齐到 32 MiB 的镜像（部分烧录卡要求 2 的幂
大小；sha1 `1c9823c650ebe60dbf18d77da84718bc01a6ce6f`）。
